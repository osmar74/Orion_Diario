...
DECLARE @FechaFiltro DATE = '{fecha_sql}';
...
WITH AsterDia_Base AS (
    SELECT Codigo, Fecha_Hora, Duracion, Estado, Atendio, Numero, Cartera
    FROM [Integral].[aster_dia_nc] a
    INNER JOIN #FiltroEntidades f ON a.entidad = f.Entidad
    WHERE a.Fecha_Hora >= @FechaInicio
      AND a.Fecha_Hora < @FechaFin
),
Bloque1_NoAtendidos AS (
    ...
    WHERE NOT (
        (_dia.Estado = 'ATENDIDO' AND _dia.Atendio = 'HUMANO')
        OR (_dia.Estado = 'ATENDIDO' AND _dia.Atendio = 'DESCONOCIDO')
    )
),
Bloque2_Contactados AS (
    ...
    LEFT JOIN [Integral].[comentarios] _contactada
        ...
        AND _contactada.fecha >= @FechaInicio
        AND _contactada.fecha < @FechaFin
...
SELECT
    ...
    CASE WHEN [Desc_Gestion] = 'Usuario pide volver a llamar' AND [Clase de Gestion] = 'codmaquina'
         THEN 'No contestan' ELSE [Desc_Gestion] END AS [Descripcion Codigo De Gestion],
    Fecha_Compromiso, Grabador, [Responsable De Cobro], Telefonos, [Tipo Cartera], Asesor, [Antiguedad De La Cartera],
    CASE WHEN [Desc_Gestion] = 'Usuario pide volver a llamar' AND [Clase de Gestion] = 'codmaquina'
         THEN 'No contestan' ELSE [Desc_Gestion] END AS [Nota de la Gestion],
    [Clase de Gestion], [Causal de Mora/Respuesta]

...
BEGIN
    ...
    DECLARE @Fecha DATE = '2026-04-29';

    

    -- Optimizamos el filtro de fecha para que sea SARGABLE (usa índices de forma nativa)
    ...
    -- 1. Extraemos y clasificamos las llamadas en una sola lectura de disco
    ...
    CTE_Aster_Base AS (
        ...
        SELECT 
            ...
            CASE WHEN Estado = 'ATENDIDO' AND Atendio IN ('HUMANO', 'DESCONOCIDO') THEN 1 ELSE 0 END AS EsHumano
    ...
    -- 2. Filtramos los comentarios del día de forma eficiente
    ...
    CTE_Comentarios_Base AS (
        ...
        WHERE fecha >= @FechaInicio AND fecha < @FechaFin
          AND ISNULL(LTRIM(RTRIM(usuario)), '') <> 'SystemUser'

    ),



    -- 3. PROCESAMIENTO: Llamadas No Atendidas por Humanos (Máquinas)
    ...
    CTE_Maquinas AS (
        ...
        SELECT DISTINCT
            ...
            CASE WHEN Codigo IS NULL OR LTRIM(RTRIM(Codigo)) = '' THEN NULL WHEN Codigo LIKE 'M%' THEN 'Mobile' ELSE 'Home' END AS [Tipo Cartera],
    ...
    -- 4. PROCESAMIENTO: Llamadas Atendidas por Humanos (Con Cruce de Comentarios)
    ...
    CTE_Humanos_Processed AS (
        ...
        SELECT 
            ...
            CASE WHEN _contactada.resultado1 IS NULL OR LTRIM(RTRIM(_contactada.resultado1)) = '' THEN 'codmaquina' ELSE 'TEL' END AS [Clase de Gestion],

            

            -- Evaluación base de la descripción
            ...
            CASE WHEN _dia.Codigo IS NULL OR LTRIM(RTRIM(_dia.Codigo)) = '' THEN NULL WHEN _dia.Codigo LIKE 'M%' THEN 'Mobile' ELSE 'Home' END AS [Tipo Cartera],

            _contactada.usuario AS Asesor,

            

            -- Limpieza de caracteres especiales centralizada

            LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(REPLACE(_contactada.comentario, CHAR(9), ''), CHAR(10), ''), CHAR(13), ''), CHAR(160), ' '))) AS [Nota_Clean],

            LTRIM(RTRIM([dbo].[Obtener_Causal_Mora](_contactada.resultado2))) AS [Causal de Mora/Respuesta]

        FROM CTE_Aster_Base _dia

        LEFT JOIN CTE_Comentarios_Base _contactada ON _dia.Codigo = _contactada.[data] 
    ...
    CTE_Humanos_Final AS (
        ...
        SELECT DISTINCT
            ...
            CASE WHEN [Desc_Raw] = 'Usuario pide volver a llamar' AND [Clase de Gestion] = 'codmaquina' THEN 'No contestan' ELSE [Desc_Raw] END AS [Descripcion Codigo De Gestion],
            ...
            CASE WHEN [Desc_Raw] = 'Usuario pide volver a llamar' AND [Clase de Gestion] = 'codmaquina' THEN '' ELSE [Nota_Clean] END AS [Nota de la Gestion],
    ...
    -- 6. SELECT FINAL CON FORMATEOS DE INTERFAZ (Se ejecutan una sola vez al final)

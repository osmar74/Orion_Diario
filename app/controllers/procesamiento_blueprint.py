...
def accion_procesar_causales():
    ...
    if res["success"]:
        ...
        if res.get("estadisticas_archivos"):
            ...
            html += "<table class='dataframe' style='width:100%;'><tr><th>Archivo</th><th>Original</th><th>Tras Campaña</th><th>Tras Evento</th><th>Normalizados</th></tr>"
...
def accion_procesar_lotes():
    ...
    if res["success"]:
        ...
        if res.get("estadisticas_archivos"):
            html += "<p style='font-size:0.75rem; color:#ccc; margin:5px 0;'>📊 Procesamiento por archivo:</p>"
            html += "<table class='dataframe' style='width:100%;'><tr><th>Archivo</th><th>Pivoteo</th><th>Filas orig.</th><th>Filas tras piv.</th><th>Cod.Cliente texto</th><th>Filas Cuenta con dato</th></tr>"
            for est in res["estadisticas_archivos"]:
                ...
                html += f"<tr><td>{est['archivo']}</td><td>{pivoteo}</td><td>{est['filas_originales']}</td><td>{est['filas_despues_pivot']}</td><td>{cod_cliente}</td><td>{cuenta_dato}</td></tr>"
            html += "</table>"
        html += "<table class='dataframe' style='width:100%; margin-top:8px;'><tr><th>Indicador</th><th>Valor</th></tr>"
        html += (
            f"<tr><td>Total filas consolidadas</td><td>{res['total_filas']}</td></tr>"
...
def accion_comparar_lotes():
    ...
    # Discador: archivo que contenga "discador" y termine en .xlsx (Consolidado)
    archivos_disc = [
        f
        for f in os.listdir(carpeta_diaria)
    ...
    # Lotes: archivo que empiece con "lote_consolidado" y termine en .xlsx
    ...
    # Causales: archivo que contenga "causales_consolidado" y termine en .xlsx
    ...
    if not ruta_disc or not ruta_caus or not ruta_lotes:
        ...
        html += f"<p style='color:#ffc107; font-size:0.75rem;'>⚠️ Falta(n) archivo(s) consolidado(s) de: {', '.join(faltantes)}. Ejecute primero los procesamientos correspondientes.</p>"
    ...
    if ruta_disc and ruta_lotes:
        try:
            ...
            if "Lote" in df_disc.columns and "Nombre_Lote" in df_lotes.columns:
                ...
                html += "<p style='font-size:0.75rem; color:#ccc; margin:10px 0 5px 0;'>🔍 Comparación de lotes:</p>"
                html += "<table class='dataframe' style='width:100%;'><tr><th>Lote</th><th>En Discador</th><th>En Lotes</th></tr>"
                for lote in todos:
                ...
                if not faltan_en_disc and not faltan_en_lotes:
                    ...
                    if faltan_en_disc:
                        html += f"<p style='color:#ffc107; font-size:0.8rem;'>⚠️ Lotes en archivo Lotes que no están en Discador: {', '.join(faltan_en_disc)}</p>"
                    if faltan_en_lotes:
                        html += f"<p style='color:#ffc107; font-size:0.8rem;'>⚠️ Lotes en Discador que no están en archivo Lotes: {', '.join(faltan_en_lotes)}</p>"
